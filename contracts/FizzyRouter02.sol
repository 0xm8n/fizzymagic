...
contract FizzyRouter02 is IFizzyRouter02 {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = FizzyLibrary.getReserves(factory, tokenA, tokenB);
    ...
    function removeLiquidityWithPermit(
        ...
        address pair = FizzyLibrary.pairFor(factory, tokenA, tokenB);
        uint value = approveMax ? MAX_UINT256 : liquidity;
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IFizzyPair(FizzyLibrary.pairFor(factory, input, output)).swap(
