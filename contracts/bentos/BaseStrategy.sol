...
abstract contract BaseStrategy is IStrategy, ExecutorAccess {
    ...
    */
    constructor(
        ...
        _grantRole(EXECUTOR_ROLE, _strategyExecutor);
    }
    ...
    function swapExactTokensForUnderlying(uint256 amountOutMin, address inputToken) public onlyExecutor returns (uint256 amountOut) {
        ...
        uint256[] memory amounts = FizzyLibrary.getAmountsOut(factory, amountIn, path, pairCodeHash);

        amountOut = amounts[amounts.length - 1];

        require(amountOut >= amountOutMin, "BentoBox Strategy: insufficient output");

        IERC20(path[0]).safeTransfer(FizzyLibrary.pairFor(factory, path[0], path[1], pairCodeHash), amounts[0]);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 ? FizzyLibrary.pairFor(factory, output, path[i + 2], pairCodeHash) : _to;
            IUniswapV2Pair(FizzyLibrary.pairFor(factory, input, output, pairCodeHash)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
}
