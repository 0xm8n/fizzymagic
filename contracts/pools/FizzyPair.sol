...
contract FizzyPair is IFizzyPair, ERC20 {
    ...
    function swap(
        ...
        require(amount0In > 0 || amount1In > 0, "ERC20: INSUFFICIENT_INPUT_AMOUNT");
        {
            ...
            require(balance0Adjusted * balance1Adjusted >= (uint256(_reserve0) * uint256(_reserve1)) * (1000**2), "ERC20: K");
        }
    ...
    function _update(
        ...
        require(balance0 <= type(uint112).max && balance1 <= type(uint112).max, "ERC20: OVERFLOW");
        uint32 blockTimestamp = uint32(block.timestamp % 2**32);
        uint32 timeElapsed = blockTimestamp - blockTimestampLast; // overflow is desired
        if (timeElapsed > 0 && _reserve0 != 0 && _reserve1 != 0) {
            // * never overflows, and + overflow is desired
            price0CumulativeLast +=
                uint256(FixedPoint.decode(FixedPoint.divuq(FixedPoint.encode(_reserve1), FixedPoint.encode(_reserve0)))) *
                timeElapsed;
            price1CumulativeLast +=
                uint256(FixedPoint.decode(FixedPoint.divuq(FixedPoint.encode(_reserve0), FixedPoint.encode(_reserve1)))) *
                timeElapsed;
        }
