...
describe("FizzyTradeToken", async () => {
    ...
    before(async () => {
        ...
        testContract = await TestContract.deploy("CAKE Trade Token","tCAKE",execAddr.address);
    ...
    it("Should transferFrom failed from not approve", async () => {
        ...
        await expect(testContract.connect(owner).transferFrom(toAddr.address,owner.address,amount))
        .to.be.reverted;
